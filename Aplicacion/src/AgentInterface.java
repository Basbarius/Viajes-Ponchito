...
public class AgentInterface extends JFrame{
    ...
    private JComboBox clienteConsultaComboBox;
    private JLabel nombreConsultasLabel;
    private JPanel queryPanel;
    private JPanel agentPanel;
    private JRadioButton crearClienteRButto;
    private JRadioButton modificarReserRButton;
    private JTextField setAtributoText;
    private JLabel modificarReserLabel;
    private JButton hacerModificacionButton;
    private JTextField atributoText;
    private JPanel hacerModificacionPanel;
    private JComboBox predicadoComboBox;
    private JLabel mensajeModificacion;
    private JLabel predicadoLabel;
    private JLabel consultaLabel;
    private JComboBox resIdComboBox;
    ...
    public AgentInterface(String activeUser, Transaction transactionManager){
        this.activeUser = activeUser;
        activeUserText.setText(this.activeUser);
        this.transactionManager = transactionManager;

        add(mainPanel);
        setTitle("Interfaz Agencia");
        setSize(940, 400);
        pack();
        ...
        llenarComboClienteConsulta(clienteConsultaComboBox);
        clienteConsultaComboBox.setSelectedItem(null);
        ...
        clienteConsultaComboBox.setEnabled(false);
        rowCount = 0;
        ...
        buttonGroup.add(crearClienteRButto);
        buttonGroup.add(modificarReserRButton);
        ...
        crearClienteRButto.addActionListener(modificacionRButtonListener);
        modificarReserRButton.addActionListener(modificacionRButtonListener);
        clienteConsultaComboBox.addActionListener(clienteTablaListener);
        hacerModificacionButton.addActionListener(hacerModificacionListener);
    }
    ...
    ActionListener paisCiudadCircuiSelectListener = new ActionListener() {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if(selecpaisciudadcircComboBox.getSelectedItem() == "Reservaciones")
            {
                paisCombo.setEnabled(false);
                paisCombo.setSelectedItem(null);
                ciudadCombo.setEnabled(false);
                ciudadCombo.setSelectedItem(null);
                clienteConsultaComboBox.setEnabled(true);
                clienteConsultaComboBox.setSelectedItem(null);
    ...
    public void llenarComboClienteConsulta(JComboBox clienteConsultaComboBox){
        clienteConsultaComboBox.removeAllItems();
            ArrayList<String[]> results = null;
            try
            {
                results = transactionManager.query("select nombreCompleto from reservacion;");
            } catch (Exception ex)
            {
                ex.printStackTrace();
            }
            ...
            {
                clienteConsultaComboBox.addItem(x[0]);
            }
            clienteConsultaComboBox.setSelectedItem(null);
    }
    ...
    ActionListener clienteTablaListener = new ActionListener() {
        @Override
        public void actionPerformed (ActionEvent e){
            drawConsultTable("cliente"); }
    };
    private void drawConsultTable(String type) {
        ...
        if(type.equals("cliente")) {
            String cliente = String.valueOf(clienteConsultaComboBox.getSelectedItem());
            String[] columnsToUse = {"Código", "Fecha simulada", "Fecha salida", "Fecha llegada", "Numero de personas", "Precio"};
            ArrayList<String[]> results = null;
            try {
                results = transactionManager.query("select r.codigoReserv, r.fechaSim, r.fechaSalida, " +
                        "r.fechaLlegada, r.numPersonas, c.precio " +
                        "from reservacion r, circuito c " +
                        "where r.nombreCompleto = \"" + cliente + "\" and r.identificadorCircuito = c.identificador;");
            } catch (Exception ex) {
                ex.printStackTrace();
            }
            if (results.isEmpty()) {
    ...
    ActionListener modificacionRButtonListener = new ActionListener() {
        @Override
        public void actionPerformed(ActionEvent e) {
            JRadioButton source = (JRadioButton) e.getSource();
            if(source.getText().equals("Crear Cliente") && source.isSelected()){
                registrationPanel.setVisible(true);
                cardnumLabel.setVisible(false);
                cardNumText.setVisible(false);
                queryPanel.setVisible(false);
            }
            else if(source.getText().equals("Modificar Reservacion") && source.isSelected()){
                registrationPanel.setVisible(false);
                queryPanel.setVisible(true);
                ArrayList<String[]> resultados = new ArrayList<>();
                predicadoComboBox.removeAllItems();
                try {
                    resultados = transactionManager.query("select codigoReserv from reservacion;");
                    for(String[] resultado : resultados){
                        predicadoComboBox.addItem(resultado[0]);
                    }
                }catch (Exception ex){
                    ex.printStackTrace();
                }
        ...
        try{
            ...
            if(!simulacionesABorrar.isEmpty()){
                for(String[] simulacionABorrar : simulacionesABorrar) {
                    ...
                    fecha = transactionManager.query("Select identificador from etapa, conformado " +
                            " where identificadoreta = identificador and identificadorCircuito = '" + simulacionABorrar[1]
                            + "';");
    ...
    ActionListener hacerModificacionListener = new ActionListener() {
        @Override
        public void actionPerformed(ActionEvent e) {
            if(registrationPanel.isVisible()){
                int year = 2021;
                ...
                try{
                    transactionManager.stmt.executeUpdate(insertQuery);
                    transactionManager.conn.commit();
                    mensajeModificacion.setText("Usuario creado correctamente");
                }
                catch(Exception ex){
                    ex.printStackTrace();
                }
            }
            else if(queryPanel.isVisible()){
                String atributo = atributoText.getText();
                String predicado = predicadoComboBox.getSelectedItem().toString();
                try{
                    transactionManager.stmt.executeUpdate("update reservacion set " + atributo + " " +
                            "where codigoreserv = '" + predicado + "';");
                    transactionManager.conn.commit();
                    mensajeModificacion.setText("Modificacion hecha correctamente");
                }
                catch(Exception ex){
                    ex.printStackTrace();
                }
